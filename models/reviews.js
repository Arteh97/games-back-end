...
exports.selectReviewById = async (review_id) => {
    return db.query('SELECT comments.review_id FROM comments LEFT JOIN reviews on reviews.review_id = comments.review_id GROUP BY comments.review_id').then(() => {
        return  db.query('SELECT reviews.*, COUNT(comments.review_id) AS comment_count FROM reviews LEFT JOIN comments ON reviews.review_id = comments.review_id WHERE reviews.review_id = $1 GROUP BY reviews.review_id;', [review_id]).then((result) => {
